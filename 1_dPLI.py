...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Calculates dPLI functional connectivity.')
    ...
    output_dir = os.path.join(args.output_dir, f'dPLI_{args.frequencyband}')
    ...
    for p_id in P_IDS:
        ...
        # plot time-averaged dPLI in PDF
        sns.heatmap(average_dpli_Base, cmap='jet', ax=axs[0], vmin=0, vmax=0.25)
        sns.heatmap(average_dpli_Anes, cmap='jet', ax=axs[1], vmin=0, vmax=0.25)
        axs[0].set_title(f"DPLI 4-8 Hz {p_id} Base")
        axs[1].set_title(f"DPLI 4-8 Hz {p_id} Anes")
        pdf.savefig(fig)

        np.save(os.path.join(output_dir, f"DPLI_{args.frequencyband}_{p_id}_Base.npy"), dpli_Base)
        np.save(os.path.join(output_dir, f"DPLI_{args.frequencyband}_{p_id}_Anes.npy"), dpli_Anes)
